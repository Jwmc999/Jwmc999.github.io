...
Let _w_ be the weight of this MST
  ...
  - Therefore $$\|\widetilde{e}| > \|e_{k}|$$ where $$\|e|$$ represents the weight of the edge _e_
...
We iterate $$\|V|-1$$ times, each time finding the _closet_ vertex
  - Iterating through the table requires is $$\Theta(|V|)$$ time
  - Each time we find a vertex, we must check all of its neighbors
  - With an adjacency matrix, the runtime is $$\Theta(|V|(|V|+|V|))=\Theta(|V|^{2})$$
  - With an adjacency list, the runtime is $$\Theta(|V|^{2}+|E|)=\Theta(|V|^{2})$$ as $$|E|=O(|V|^{2})$$
...
We iterate $$|V|-1$$ times, each time finding the _closest_ vertex
...
The each edge visited may result in a new edge being pushed to the very top of the heap. Thus, the work required for this is $$O(|E| \lg(|V|))$$  
Total runtime: $$O(|V| \lg(|V|)+ |E| \lg(|V|))=O(|E| \lg(|V|))$$
