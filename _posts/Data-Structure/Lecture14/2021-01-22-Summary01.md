...
On this graph, iterate the following $$\left|V\right|$$ = 12 times
...
- This requires : $$\Theta(\left|V\right|)$$ memory

Iterate at least $$\left|V\right|$$ times, so the runtime must be : $$\Omega(\left|V\right|)$$
* Find vertices with in-degree zero
- loop through the array with each iteration
- Runtime: $$O(\left|V\right|^{2})$$
...
- Scan through each of the vertices : $$\Theta(\left|V\right|)$$
- For each vertex, we will have to push onto and pop off the queue once, also $$\Theta(\left|V\right|)$$

Each value in the in-degree table is associated with an edge
- Here, $$\left|E\right|$$=16
- Each of the in-degrees must be decremented to zero
- The runtime of these operations is $$\Omega(\left|E\right|)$$
- If we are using an adjacency matrix: $$\Theta(\left|V\right|^{2})$$
- If we are using an adjacency list: $$\Theta(\left|E\right|)$$

Total runtime:
- If we use an adjacency list: $$\Theta(\left|V\right|+\left|E\right|)$$
- If we use an adjacency matrix: $$\Theta(\left|V\right|^{2})$$
- Memory requirement: $$\Theta(\left|V\right|)$$
